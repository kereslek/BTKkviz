...
export default function Home() {
  ...
  // Feedback for showing correct answer green after wrong
  const [showFeedback, setShowFeedback] = useState(false);

  // Chat - closed by default on mobile, open on desktop
  const [chatOpen, setChatOpen] = useState(true); // Will be overridden in useEffect for mobile
  ...
  // Fallback UUID generator (fixes iPhone/Safari crypto.randomUUID error)
  const generateUUID = () => {
  ...
  // Set chat open state based on screen size (mobile closed by default)
  useEffect(() => {
    const checkMobile = () => {
      if (window.innerWidth < 768) { // md breakpoint in Tailwind
        setChatOpen(false); // Closed on mobile
      } else {
        setChatOpen(true); // Open on desktop
      }
    };
    ...
    const loadMessages = async () => {
      ...
      if (error) setChatError('Betöltési hiba: ' + error.message);
        ...
        if (messages.length === 0) {
          messages = [
            { id: generateUUID(), nickname: 'Admin', message: 'Üdv a BTK kvízben! Készen állsz a kihívásra? 🔥', created_at: new Date().toISOString() },
            { id: generateUUID(), nickname: 'Játékos1', message: 'Szia! Indulhat a játék!', created_at: new Date().toISOString() },
          ];
        }
  ...
  const startGame = () => {
    console.log('startGame called - resetting state');
    setQuestions([]);
    setCurrentIndex(-1);
    setScore(0);
    setStreak(0);
  ...
  const loadNextQuestion = () => {
    console.log('=== loadNextQuestion START ===');
    console.log('criminals total:', criminals.length);
    console.log('usedCriminalIds size:', usedCriminalIds.size);
    console.log('questions length (before):', questions.length);

    setShowFeedback(false);

    if (questions.length >= 10) {
      console.log('Game over - 10 questions reached');
      setEndTime(Date.now());
      setGameOver(true);
      fetchLeaderboard();
      return;
    ...
    if (criminals.length < 4) {
      console.log('ERROR: Not enough criminals (<4)');
      return;
    }

    try {
      let available = criminals.filter(c => 
        c.photo_url && typeof c.photo_url === 'string' && c.photo_url.trim() !== '' && 
        c.id && typeof c.id === 'string' && !usedCriminalIds.has(c.id)
      );

      console.log('Available after strict filter:', available.length);

      if (available.length === 0) {
        console.log('No available - resetting used IDs');
        setUsedCriminalIds(new Set());
        setUsedCrimesGlobal(new Set());
        available = criminals.filter(c => 
          c.photo_url && typeof c.photo_url === 'string' && c.photo_url.trim() !== '' && 
          c.id && typeof c.id === 'string'
        );
        console.log('Available after reset:', available.length);
        if (available.length < 4) {
          console.log('ERROR: Still not enough after reset');
          return;
        }
      }
      ...
      if (!correct || !correct.id || !correct.crime) {
        console.log('ERROR: Selected criminal invalid', correct);
        return;
      }

      console.log('Selected correct criminal:', correct.id, correct.name || 'no name', correct.crime);

      setUsedCriminalIds(prev => {
        const newSet = new Set([...prev, correct.id]);
        console.log('Updated usedCriminalIds size:', newSet.size);
        return newSet;
      });

      setUsedCrimesGlobal(prev => {
        const newSet = new Set([...prev, correct.crime]);
        console.log('Updated usedCrimesGlobal size:', newSet.size);
        return newSet;
      });
      ...
      console.log('Wrong pool size:', wrongPool.length);
      ...
      console.log('Wrong crimes selected:', wrongCrimes);
      ...
      console.log('Creating new question #', newQuestion.questionNumber, '-', correct.name || 'no name');

      // Update questions first
      setQuestions(prev => {
        const updated = [...prev, newQuestion];
        console.log('setQuestions - new length:', updated.length);

        // Then set currentIndex to the NEW last index
      ...
      console.error('CRITICAL ERROR in loadNextQuestion:', err);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#001f3f] to-[#0a2540] text-white flex flex-col relative font-sans">
      ...
      <div className="bg-red-800 p-4 text-center font-bold text-base shadow-lg">
        Ez kizárólag szórakoztató és oktatási célű kvíz.
      </div>
          ...
          <div className="max-w-5xl mx-auto">
            ...
            {current && current.criminal ? (
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-8 md:p-12 rounded-3xl shadow-2xl border border-gray-700 relative">
                ...
                {/* Back/Forward buttons below options */}
                <div className="flex justify-center gap-8 mt-10 mb-6">
      ...
      <footer className="bg-gradient-to-t from-red-950 to-red-900 p-8 text-center text-lg mt-auto shadow-inner">
        Ez kizárólag szórakoztató és oktatási célű kvízjáték.  
        Az oldal semmilyen kapcsolatban nem áll a hatóságokkal,  
        és semmilyen valós jogi vagy hatósági eljárásra nem használható.
      </footer>
    </div>
  );
}