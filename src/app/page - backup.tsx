...
const [usedCriminalIds, setUsedCriminalIds] = useState<Set<string>>(new Set());
  ...
  const [showFeedback, setShowFeedback] = useState(false); // ← NEW LINE
  ...
  const handleAnswer = (answer: string) => {
    ...
    if (isCorrect) {
      ...
      setStreak(0);
      setShowFeedback(true); // Show correct answer green for 0.2 seconds
    }

    setTimeout(() => {
      setShowFeedback(false); // Hide feedback after 0.2 seconds
      if (mode === '10' && questions.length >= 10) {
        setEndTime(Date.now());
        setGameOver(true);
        fetchLeaderboard();
