...
export default function Home() {
  ...
  // Clean crime name (remove " - BTK ..." part)
  ...
  // Live players count with Supabase Presence
  useEffect(() => {
  ...
  const loadNextQuestion = () => {
    ...
    // FIX: chain the index update inside setQuestions so it uses the NEW length
    setQuestions(prev => {
      const updated = [...prev, newQuestion];
      const newIndex = updated.length - 1;
      setCurrentIndex(newIndex); // now correct index after update
      return updated;
    });
  };
  ...
  const generateQuestionShare = async () => {
    ...
    try {
      if (navigator.share) {
        ...
        const fallbackText = `${shareText}\n\nJátssz most: ${window.location.origin}?c=${current.criminal.id}`;
        navigator.clipboard.writeText(fallbackText);
        alert('Link és szöveg kimásolva!');
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#001f3f] to-[#0a2540] text-white flex flex-col relative font-sans">
        ...
        {!gameOver ? (
          <div className="max-w-4xl mx-auto">
              ...
              <div className="bg-gradient-to-br from-gray-900 to-gray-800 p-5 md:p-8 rounded-3xl shadow-2xl border border-gray-700 relative">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-5">
                  {current.options.map((opt, idx) => {
                    ...
                    // Wrong selected stays red during feedback
                    if (isSelected && !isCorrect) {
                      buttonClass += ' bg-red-600 text-white border-red-400 ring-4 ring-red-300/50';
                    }
                    // Correct selected or feedback correct stays green
                    ...
                    // Clickable but not selected/feedback
                    else if (isLatest) {
            ...
            <div className="mt-10 md:mt-12 text-center">
              <button onClick={startGame} className="bg-gradient-to-r from-red-600 via-orange-500 to-yellow-500 hover:from-red-700 hover:via-orange-600 hover:to-yellow-600 px-10 md:px-12 py-5 md:py-6 rounded-3xl font-extrabold text-2xl md:text-3xl shadow-2xl transform hover:scale-105 transition-all duration-300">
                ÚJ JÁTÉK
              </button>
            </div>
      ...
      <div className="text-center py-4 text-lg font-medium text-gray-300">
        Jelenleg online: {onlinePlayers} játékos
      </div>
      ...
      <footer className="bg-gradient-to-t from-red-950 to-red-900 p-6 md:p-8 text-center text-base md:text-lg mt-auto shadow-inner">
        Ez kizárólag szórakoztató és oktatási célű kvízjáték, emellett a hatóságok munkájának segítése és a közbiztonsági tudatosság növelése céljából is.
      </footer>
    </div>
  );
}